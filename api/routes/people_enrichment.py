...
def get_enrichment_status(enrichment_id: str):
    ...
    try:
        ...
        # The job is complete if the response has a 'people' array
        if result and 'people' in result:
            return jsonify({"success": True, "status": "completed", "data": result['people']})
        
        # Otherwise, the job is still pending
        return jsonify({"success": True, "status": "pending", "data": result})

    except Exception as e:
        logger.error(f"Failed to get people enrichment status for ID {enrichment_id}: {e}")
