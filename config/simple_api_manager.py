...
class SimpleApiManager:
    ...
    def add_dynamic_key(self, key_name: str, api_key: str) -> bool:
        ...
        logger.info(f"Added dynamic API key: {key_name} (...{api_key[-5:]})")
        return True
    ...
    def _load_env_keys(self) -> Dict[str, ApiKey]:
        ...
        for pattern in env_patterns:
            ...
            if key_value and key_value.strip():
                ...
                keys_found += 1
                logger.info(f"Loaded API key: {key_name} (...{key_value[-5:]})")
    ...
    def get_all_keys(self) -> List[Dict]:
        ...
        return [
            {
                'name': key_obj.name,
                'key_suffix': key_obj.key[-5:] if len(key_obj.key) >= 5 else key_obj.key,
